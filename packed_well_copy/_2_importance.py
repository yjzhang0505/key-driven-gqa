...
# def calculate_stats(model):
#     """
#     计算每层每个头的 Q、K、V 权重的均值和方差，并对均值和方差矩阵进行归一化，使其均值为 1。
#     """
#     all_stats = {'K': [], 'Q': [], 'V': [], 'KxQ': [], 'KxQxV': []}
...
def calculate_singular_values(model):
    """
    计算每层每个头的 Q、K、V 权重的奇异值，以及 K * Q 和 K * Q * V 的奇异值，直接保存奇异值本身。
    """
    # 初始化字典来存储奇异值
    stats_tensor = {
        'K_singular': [],
    ...
    for block_idx in range(model.num_layers):
        ...
        # 将权重 reshape 成 (num_heads, dim_per_head, dim) 的形式
        dim_per_head = model.dim // model.num_heads
        q_weight_heads = q_weight.view(model.num_heads, dim_per_head, model.dim)
        k_weight_heads = k_weight.view(model.num_heads, dim_per_head, model.dim)
        v_weight_heads = v_weight.view(model.num_heads, dim_per_head, model.dim)
        ...
        for i in range(model.num_heads):
            ...
            # 将每个头的奇异值直接保存到对应的列表中
            stats_tensor['K_singular'].append(k_singular_values)
            stats_tensor['Q_singular'].append(q_singular_values)
            stats_tensor['V_singular'].append(v_singular_values)
            stats_tensor['KxQ_singular'].append(kq_singular_values)
            stats_tensor['KxQxV_singular'].append(kqv_singular_values)

    # 将奇异值列表转换为张量，并按每层有 12 个头的形式 reshape
    stats_tensor = {key: torch.stack(value).view(-1, 12, value[0].shape[0]) for key, value in stats_tensor.items()}
