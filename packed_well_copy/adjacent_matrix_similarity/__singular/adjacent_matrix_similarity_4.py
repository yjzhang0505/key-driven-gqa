...
def similarity_score_matrix(A, B):
    """
    计算矩阵 A 和 B 之间的图编辑距离相似性。
    返回单一的相似度分数，表示 A 和 B 的编辑距离相似性（距离越小，相似度越高）。
    """
    ...
    # 平方和根的相反数作为相似性，以距离越小相似性越高
    edit_distance = np.sqrt(np.sum((A - B) ** 2))
    similarity_score = -edit_distance  # 越接近0越相似
...
plot_adjacency_matrix_graph(adj_matrix_1, "adj_matrix_1", iteration="initial", output_dir="/data/yjzhang/desktop/try/not_share/key-driven-gqa/figure/figure_2/adjacent_matrix_4/figure")
...
# 设置保存文件的基础目录
output_dir = '/data/yjzhang/desktop/try/not_share/key-driven-gqa/figure/figure_2/adjacent_matrix_4/files'
figure_dir = '/data/yjzhang/desktop/try/not_share/key-driven-gqa/figure/figure_2/adjacent_matrix_4/figure'
