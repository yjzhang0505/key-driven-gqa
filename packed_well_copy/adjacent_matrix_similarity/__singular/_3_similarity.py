...
def cosine_similarity_matrix(A, B):
    ...
    for i in range(n):
        for j in range(n):
            ...
            similarity_matrix[i, j] = sum(dim_similarities) / m
...
def cosine_similarity(u, v):
    """计算两个向量之间的相似度，定义为向量差的模长除以两个向量模的乘积"""
    # 计算向量差的模长
    norm_difference = np.linalg.norm(u - v)
    ...
    # 计算相似度（1 - 差的模长除以模的乘积，确保范围在[0,1]之间）
    return 1 - (norm_difference / (norm_u * norm_v))
...
def calculate_similarity(model):
    ...
    for block_idx in range(model.num_layers):
        ...
        similarity_matrices = {
            'K_similarity': cosine_similarity_matrix(k_weight_heads, k_weight_heads),
            'Q_similarity': cosine_similarity_matrix(q_weight_heads, q_weight_heads),
            'V_similarity': cosine_similarity_matrix(v_weight_heads, v_weight_heads),
            'KxQ_similarity': cosine_similarity_matrix(
                torch.matmul(k_weight_heads, q_weight_heads.transpose(-2, -1)),
                torch.matmul(k_weight_heads, q_weight_heads.transpose(-2, -1))
            ),
            'KxQxV_similarity': cosine_similarity_matrix(
