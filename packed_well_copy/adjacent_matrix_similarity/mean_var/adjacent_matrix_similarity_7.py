...
def similarity_score_matrix(A, B):
    """
    使用 Wasserstein 距离计算矩阵 A 和 B 的相似性。
    返回一个相似性分数，距离越小，表示分布越相似。
    """
    ...
    # 将矩阵的边权看作分布，展开为一维
    A_flat = A[np.triu_indices_from(A, k=1)]
    B_flat = B[np.triu_indices_from(B, k=1)]

    # 计算 Wasserstein 距离，距离越小表示分布越接近
    similarity_score = -wasserstein_distance(A_flat, B_flat)
...
plot_adjacency_matrix_graph(adj_matrix_1, "adj_matrix_1", iteration="initial", output_dir="/data/yjzhang/desktop/try/not_share/key-driven-gqa/figure/adjacent_matrix_7/figure")
...
# 设置保存文件的基础目录
output_dir = '/data/yjzhang/desktop/try/not_share/key-driven-gqa/figure/adjacent_matrix_7/files'
figure_dir = '/data/yjzhang/desktop/try/not_share/key-driven-gqa/figure/adjacent_matrix_7/figure'
