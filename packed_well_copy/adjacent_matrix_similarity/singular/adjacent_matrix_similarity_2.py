...
def similarity_score_matrix(A, B, K=10):
    """
    使用 Top-K 重叠度量计算矩阵 A 和 B 的相似性。
    返回一个单一的相似度分数，表示 A 和 B 中权重最高的前 K 条边的重叠比例。
    """
    ...
    # 获取 A 和 B 中权重最高的前 K 条边的索引
    top_k_edges_A = get_top_k_edges(A, K)
    top_k_edges_B = get_top_k_edges(B, K)

    # 计算 A 和 B 的前 K 条边之间的重叠
    overlap_count = len(set(top_k_edges_A) & set(top_k_edges_B))
    similarity_score = overlap_count / K  # 重叠比例

    return similarity_score

def get_top_k_edges(matrix, K):
    """
    获取矩阵中权重最高的前 K 条边的索引。
    返回值为一组元组，每个元组表示一条边的 (i, j) 索引。
    """
    # 将上三角（不含对角线）的边提取出来，并按权重排序
    indices = np.triu_indices_from(matrix, k=1)
    weights = matrix[indices]
    top_k_indices = np.argsort(weights)[-K:]  # 取出权重最高的 K 条边的索引

    # 将一维索引转换为二维索引（i, j）形式
    top_k_edges = [(indices[0][idx], indices[1][idx]) for idx in top_k_indices]
...
plot_adjacency_matrix_graph(adj_matrix_1, "adj_matrix_1", iteration="initial", output_dir="/data/yjzhang/desktop/try/not_share/key-driven-gqa/figure/figure_2/adjacent_matrix_2/figure")
...
# 设置保存文件的基础目录
output_dir = '/data/yjzhang/desktop/try/not_share/key-driven-gqa/figure/figure_2/adjacent_matrix_2/files'
figure_dir = '/data/yjzhang/desktop/try/not_share/key-driven-gqa/figure/figure_2/adjacent_matrix_2/figure'
