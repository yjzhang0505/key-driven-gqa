...
def load_singular_values_from_model(model, layer_index):
    ...
    """
    # 使用 calculate_singular_values 函数从模型中获取重要性评分
    importance_scores = calculate_singular_values(model, layer_index)
    ...
    # 由于重要性评分只包含 K 的奇异值，这里你需要提供 Q、V 等的值
    # 如果你想计算 Q、V 的重要性评分，可以使用相应的计算方法

    # 假设 Q 和 V 的重要性评分和 K 的相同（你可能需要根据实际情况进行修改）
    for head in range(12):
        singulars_tensor['K_singular'].append(importance_scores[head])  # K 的重要性评分
        singulars_tensor['Q_singular'].append(importance_scores[head])  # Q 的重要性评分（占位）
        singulars_tensor['V_singular'].append(importance_scores[head])  # V 的重要性评分（占位）
        singulars_tensor['KxQ_singular'].append(importance_scores[head])  # KxQ 的重要性评分（占位）
        singulars_tensor['KxQxV_singular'].append(importance_scores[head])  # KxQxV 的重要性评分（占位）
