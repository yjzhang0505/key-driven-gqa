...
def similarity_score_matrix(A, B, top_k=5):
    """
    计算矩阵 A 和 B 之间的谱相似性。返回特征值差异的相似性得分。
    """
    ...
    L_A = D_A - A
    D_B = np.diag(B.sum(axis=1))
    L_B = D_B - B

    # 计算前 K 个特征值
    eigvals_A = np.linalg.eigvalsh(L_A)[:top_k]
    eigvals_B = np.linalg.eigvalsh(L_B)[:top_k]

    # 使用欧式距离衡量特征值差异
    spectral_distance = euclidean(eigvals_A, eigvals_B)
    similarity_score = -spectral_distance  # 越接近0越相似
...
plot_adjacency_matrix_graph(adj_matrix_1, "adj_matrix_1", iteration="initial", output_dir="/data/yjzhang/desktop/try/not_share/key-driven-gqa/figure/figure_2/adjacent_matrix_5/figure")
...
# 设置保存文件的基础目录
output_dir = '/data/yjzhang/desktop/try/not_share/key-driven-gqa/figure/figure_2/adjacent_matrix_5/files'
figure_dir = '/data/yjzhang/desktop/try/not_share/key-driven-gqa/figure/figure_2/adjacent_matrix_5/figure'
