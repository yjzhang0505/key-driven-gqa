...
def calculate_singular_values(model, layer_idx):
    """
    计算指定层每个头的 Q、K、V 权重的奇异值，以及 K * Q 和 K * Q * V 的奇异值。
    """
    importance_scores = []
    ...
    for i in range(model.num_heads):
        ...
        # 取前五个最大奇异值
        top_k_singular_values = k_singular_values[:5]

        # 进行指数平滑
        alpha = 0.3  # 平滑因子，范围在 0 到 1 之间
        smoothed_value = top_k_singular_values[0]  # 初始值
        for singular_value in top_k_singular_values[1:]:
            smoothed_value = alpha * singular_value + (1 - alpha) * smoothed_value
        
        # 保存每个头的重要性评分
        importance_scores.append(smoothed_value.item())

    return importance_scores  # 返回12个头对应的重要性评分
