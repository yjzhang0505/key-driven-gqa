...
def calculate_similarity(model):
    ...
    for block_idx in range(model.num_layers):
        ...
        similarity_matrices = {
            'K_cosine': cosine_similarity_matrix(k_weight_heads, k_weight_heads),
            'Q_cosine': cosine_similarity_matrix(q_weight_heads, q_weight_heads),
            'V_cosine': cosine_similarity_matrix(v_weight_heads, v_weight_heads),
            'KxQ_cosine': cosine_similarity_matrix(
                torch.matmul(k_weight_heads, q_weight_heads.transpose(-2, -1)),
                torch.matmul(k_weight_heads, q_weight_heads.transpose(-2, -1))
            ),
            'KxQxV_cosine': cosine_similarity_matrix(
                torch.matmul(torch.matmul(k_weight_heads, q_weight_heads.transpose(-2, -1)), v_weight_heads),
                torch.matmul(torch.matmul(k_weight_heads, q_weight_heads.transpose(-2, -1)), v_weight_heads)
            )
        }
