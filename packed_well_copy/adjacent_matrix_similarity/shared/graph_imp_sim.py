...
def combine_matrices_2(importance, similarity_matrix, weight_importance=0.6, weight_similarity=0.4):
    # 将 importance 转换为一维向量
    importance = importance.flatten()
    
    # 生成方阵并转换为 torch.Tensor
    importance_matrix = torch.tensor(np.outer(importance, importance) * 3)
...
def generate_adjacency_matrices_2(model, weight_importance=0.6, weight_similarity=0.4,
                                singularity_key='K_singular', similarity_key='K_similarity'):
    # 计算相似性矩阵
    similarity_matrices = calculate_similarity(model)
    similarity_dir = '/data/yjzhang/desktop/try/not_share/key-driven-gqa/figure/dustbin/similarity.txt'
    # similarity_matrices = similarity_matrices.tolist()
    save_to_txt(similarity_dir,similarity_matrices)
    layer_index = 0
    importance_matrix = load_singular_values_from_model(model, layer_index)
    importance_dir = '/data/yjzhang/desktop/try/not_share/key-driven-gqa/figure/dustbin/importance.txt'
    # importance_matrix = importance_matrix.tolist()
    save_to_txt(importance_dir,importance_matrix)    

    # 从第0层提取指定的相似性矩阵
    layer_similarity = similarity_matrices.get(f'Layer_{layer_index}', {})
    similarity_matrix = layer_similarity.get(similarity_key)
    # importance_matrix1 = importance_matrix['K_singular'] 
    # print(importance_matrix1.shape)
