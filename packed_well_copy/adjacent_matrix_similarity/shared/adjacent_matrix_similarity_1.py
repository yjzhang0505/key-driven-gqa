...
def similarity_score_matrix(A, B):
    ...
    sim_AB = 0
    for i in range(n):
        pearson_line = pearson_similarity(A[i], B[i])  # 使用皮尔逊相关系数计算行相似度
        pearson_row = pearson_similarity(A[:, i], B[:, i])  # 使用皮尔逊相关系数计算列相似度
        sim_AB += pearson_line + pearson_row
    sim_AB /= (2 * n)
    return sim_AB
...
def pearson_similarity(u, v):
    ...
    correlation = np.corrcoef(u, v)[0, 1]
    return correlation if not np.isnan(correlation) else 0  # 如果相关系数为 NaN，则返回 0
...
plot_adjacency_matrix_graph(adj_matrix_1, "adj_matrix_1", iteration="initial", output_dir="/data/yjzhang/desktop/try/not_share/key-driven-gqa/figure/adjacent_matrix_1/figure")
...
# 设置保存文件的基础目录
output_dir = '/data/yjzhang/desktop/try/not_share/key-driven-gqa/figure/adjacent_matrix_1/files'
figure_dir = '/data/yjzhang/desktop/try/not_share/key-driven-gqa/figure/adjacent_matrix_1/figure'
