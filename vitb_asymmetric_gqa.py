...
class Attention(nn.Module):
    def __init__(
        ...
        for row in input_data:
            group_sizes = [len(group) for group in row]
            all_group.append(group_sizes)

        # # 输出分组情况
        # print("self.all_group =", all_group)
        ...
        self.group_sizes = all_group[layer_index]
    ...
    def att_weight_conversion(self, qkv_params, block_idx, is_bias=False):
        ...
        def convert_weight(param):
            ...
            xs1 = torch.split(x, self.group_sizes, dim=0)  # 按打乱后的分组进行分割

            # # 3 创建权重向量，假设 num_kv_heads 是分组的数量
            # weights = torch.linspace(self.num_kv_heads, 1, steps=self.num_kv_heads)  # 递减的权重
            # weights = weights / weights.sum()  # 归一化使权重总和为1

            # # 按打乱后的分组进行分割
            # xs = torch.split(x, self.group_sizes, dim=0)

            # # 计算加权平均
            # xs = [torch.sum(xs1[i] * weights[i], dim=0) for i in range(self.num_kv_heads)]

            #1
            xs = [xs1[i].mean(dim=0) for i in range(self.num_kv_heads)]  # 平均池化
            
            # #2 从每个分组中取出第一个元素
            # xs = [xs1[i][0] for i in range(self.num_kv_heads)]
