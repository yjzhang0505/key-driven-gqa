...
def get_model(
    ...
    '''
    args = dict(num_classes=num_classes, pretrained=pretrained, att_scheme=att_scheme, window_size=window_size, num_kv_heads=num_kv_heads, exp_num=exp_num, in_chans=in_chans, filename=filename)
    if size == 's':
...
def train_step(model, dataloader, criterion, optimizer, device):
    ...
    for step, (X, y) in progress_bar:
        ...
        # 前向传播
        logits = model(X)
        # print(logits.shape)  # 应该是 (batch_size, 100)
...
if __name__ == "__main__":
    ...
    #             print(f"Test accuracy has not improved for {early_stop_patience} epochs. Stopping early.")
    #             break
