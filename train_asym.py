...
from hooks import register_hooks, hook_fn
...
args.file_path = "/data/yjzhang/desktop/try/not_share/key-driven-gqa/output/dustbin2/group_by_similarity/group_66.txt"
...
# 检查点路径
pth_path = "/data/yjzhang/desktop/try/ckpt/vitb_ok.pth"  # 替换为你的检查点文件路径
...
new_file_name = "Debug_proxy=1.txt"
...
def compute_hessian_without_training(model, input_data):
    """
    计算海森矩阵，不经过训练。
    1. 进行前向传播，获取输出。
    2. 计算一阶梯度。
...
# 示例：计算 Vision Transformer 模型的海森矩阵
# model = VisionTransformer(embed_dim=768, num_heads=12, num_blocks=12)

# 输入数据
input_data = torch.randn(1, 2, 384, 768)  # 假设 batch_size=2, sequence_length=10, embed_dim=768

# 计算海森矩阵
hessian = compute_hessian_without_training(model, input_data)

# 打印海森矩阵的内容
for param, hessian_matrix in hessian.items():
    print(f"Hessian for parameter {param.shape}:")
