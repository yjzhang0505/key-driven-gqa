...
def plot_k_weights_heatmaps(k_file, num_heads, dim, output_image_path, target_sum=1.0):
    ...
    # 将平铺数据转换为张量，并 reshape 为 (num_heads, dim_per_head, dim_per_token)
    total_elements = len(k_weight_flat)
    dim_per_head = total_elements // (num_heads * dim)  # 计算每个head的dim_per_token
    k_weight = torch.tensor(k_weight_flat).view(num_heads, dim_per_head, dim)
    ...
    for i in range(num_heads):
        ...
        # 对缩放后的数据应用指数函数 e^x
        exp_head_data = torch.exp(10*scaled_head_data)
...
dim = 512       # 假设每个头的维度为512（已调整）

# 输出图片路径
output_image_path = '/data/yjzhang/desktop/try/key-driven-gqa/data/finetuned/k_weights_exp_scaled_heatmap_green.png'
