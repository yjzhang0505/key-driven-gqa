...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 使用分别的 Linear 层处理 q, k, v
        q = self.q(x).view(B, N, self.num_heads, self.head_dim).transpose(1, 2)  # (B, H, N, head_dim)
        k = self.k(x).view(B, N, self.num_heads, self.head_dim).transpose(1, 2)  # (B, H, N, head_dim)
        v = self.v(x).view(B, N, self.num_heads, self.head_dim).transpose(1, 2)  # (B, H, N, head_dim)
...
class VisionTransformer(nn.Module):
    ...
    def load_pretrained_weights(self, state_dict):
        """
        Function to load pretrained weights from a state_dict
        """
