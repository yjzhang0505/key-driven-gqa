...
folders_more = list(range(101,1000))
...
with open(output_file, 'w') as outfile:
    ...
    for group_line, group_path in all_group_lines_with_paths:
        ...
        # 如果该分组已经存在，追加路径
        if formatted_line in seen_groups:
            seen_groups[formatted_line].append(group_path)
        else:
            # 否则记录该分组和其路径
            seen_groups[formatted_line] = [group_path]
    
    # 将结果写入文件
    for formatted_line, group_paths in seen_groups.items():
        # 将所有路径用逗号连接
...
# output_file = '/data/yjzhang/desktop/try/key-driven-gqa/calculate/group_more_aline.txt'
